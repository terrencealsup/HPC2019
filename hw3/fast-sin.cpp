...
sin4_vector() now computes to 12 digits of accuracy.
**/
#include <math.h>
#include <stdio.h>
#include "utils.h"
...
void sin4_vector(double* sinx, const double* x) {
  ...
  // We just need to add the higher order terms x^5, x^7, x^9, and x^11.
  ...
  // Now add the higher order terms in the Taylor approximation.
  Vec4 s = x1;
  s += x3  * c3 ;
  s += x5  * c5 ;
  s += x7  * c7 ;
