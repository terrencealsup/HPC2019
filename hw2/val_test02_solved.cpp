...
The only change was:
Initialize the remaining values x[5] to x[9].

These values are called later in the program, which is why they must be
initialized.
*/
...
// Only the first 5 values of x are being initialized.
  for ( i = 0; i < 5; i++ )
  {
    x[i] = i;
  }

// We just need to initialize the remaining values.
  for ( i = 5; i < 10; i++ )
  {
    x[i] = 0;
  }
